...
Position askPostionHumain(Symboles jeu[TAILLEJEU][TAILLEJEU], TPlayer p, Position* TabPosition)
{
    Position ChoosenPos;
    int X = 0;
    int Y = 0;
...
Position bestPositionToPlay(QList list, Symboles symbole, int* recupValMax)
{
    ...
    while(!isEmpty_QL(parcours))
    {
        ...
        {
             int valQ = q->valquintuplet + getVal(getQ_QL(parcours)->pos[i],vide) +  getVal(getQ_QL(parcours)->pos[i],adv) + getVal(getQ_QL(parcours)->pos[i],sym) ;
...
static Position tourHumain(TPlayer* PlayPlayer, Symboles jeu[TAILLEJEU][TAILLEJEU], Position* tabPosition)
{
    return askPostionHumain(jeu, *PlayPlayer, tabPosition);
}
...
void tourPlayer(TPlayer* PlayPlayer, TPlayer* WaitPlayer, QList* mainList, Symboles jeu[TAILLEJEU][TAILLEJEU], Position* tabPosition)
{
    ...
    if (PlayPlayer->typejoueur == Humain)
    {
        ...
        posToPlay = tourHumain(PlayPlayer, jeu, tabPosition);
