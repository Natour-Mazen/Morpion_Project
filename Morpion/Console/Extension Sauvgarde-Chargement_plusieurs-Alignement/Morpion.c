...
// Regarde si "action" est dans la liste de keys pour continuer le jeu.
bool continueKeysAction(char action)
{
    return (action == 'n' || action == 'N');
}

// Regarde si "action" est dans la liste de keys pour sauvegarder en binaire.
bool saveBinaryKeysAction(char action)
{
    return (action == 's' || action == 'S');
}

// Regarde si "action" est dans la liste de keys pour charger une partie en binaire.
bool loadBinaryKeysAction(char action)
{
    return (action == 'l' || action == 'L');
}

// Regarde si "action" est dans la liste de keys pour sauvegarder en texte.
bool saveTextKeysAction(char action)
{
    return (action == 'd' || action == 'D');
}

// Regarde si "action" est dans la liste de keys pour charger une partie en texte.
bool loadTextKeysAction(char action)
{
    return (action == 'm' || action == 'M');
}

// Regarde si "action" est dans la liste de keys.
bool allKeysAction(char action)
{
    return (quitKeysAction(action) || continueKeysAction(action)
                                   || saveBinaryKeysAction(action) || loadBinaryKeysAction(action) 
                                   || saveTextKeysAction(action) || loadTextKeysAction(action));
}
...
Position askPostionHumain(Symboles jeu[TAILLEJEU][TAILLEJEU], TPlayer p, TPlayer otherP, Position* TabPosition, char* instruction)
{
    ...
    ChoosenPos = readPositionToPlay(p, otherP, &X, &Y, TabPosition, jeu);
...
Position bestPositionToPlay(QList list, Symboles symbole, int* recupValMax)
{
    ...
    while(!isEmpty_QL(parcours))
    {
        ...
        {
             int valQ = q->valquintuplet + getVal(getQ_QL(parcours)->pos[i],vide) +  getVal(getQ_QL(parcours)->pos[i],adv) + getVal(getQ_QL(parcours)->pos[i],sym) ;            // meilleur position sans symbole et en fonction de qui fait l'appel ajout de getSymbole
...
static Position tourHumain(TPlayer* PlayPlayer, TPlayer* WaitPlayer, Symboles jeu[TAILLEJEU][TAILLEJEU], Position* tabPosition, char* instruction)
{
    return askPostionHumain(jeu, *PlayPlayer, *WaitPlayer, tabPosition, instruction);
}

// Tour de jeu pour un Bot.
static Position tourBot(TPlayer* PlayPlayer, TPlayer* WaitPlayer, QList* mainList, Symboles jeu[TAILLEJEU][TAILLEJEU], Position* tabPosition)
{
    ...
    askPositionBot(jeu, posToPlay, *PlayPlayer, *WaitPlayer, tabPosition);
    return posToPlay;
}
...
void tourPlayer(TPlayer* PlayPlayer, TPlayer* WaitPlayer, QList* mainList, Symboles jeu[TAILLEJEU][TAILLEJEU], Position* tabPosition, char* instruction)
{
    ...
    WaitPlayer->listeGagnant = removeClosedQuints(WaitPlayer->listeGagnant);
    ...
    if (PlayPlayer->typejoueur == Humain)
    {
        ...
        posToPlay = tourHumain(PlayPlayer, WaitPlayer, jeu, tabPosition, instruction);
        
    }
    // Si PlayPlayer est un bot.
    else
    {
        posToPlay = tourBot(PlayPlayer, WaitPlayer, mainList, jeu, tabPosition);
    }
