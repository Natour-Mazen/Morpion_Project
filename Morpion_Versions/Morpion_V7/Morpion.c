...
listequintuplet removeClosedQuints(listequintuplet l)
{
    ...
    while (courant != NULL && courant->suiv != NULL)
    {
        ...
        for (int i = 0 ; i < NTUPLET ; i++)
        {
            ...
            if (courant->Qdata->pos[i]->Sy == rond){
                oExist = true;
            }

            if (courant->Qdata->pos[i]->Sy == croix){
                xExist = true;
            }
        }

        //test
        printf("tour dans remove =%d, oExist = %d et xExist = %d\n",tour , oExist,xExist);
        tour++;
        ...
        if (oExist && xExist)
        {
            ...
            if(aSuppr->suiv == NULL && aSuppr->prec != NULL)
            {
                aSuppr->prec->suiv = NULL;
            }

            if(aSuppr->prec == NULL && aSuppr->suiv != NULL) //si c'est le premier quintuplet de la liste, on pense a faire les
            {
                aSuppr->suiv->prec = NULL;
            }

            //si c'est pas le cas, on fait la même chose différemment
            if(aSuppr->suiv != NULL && aSuppr->prec !=NULL)
            {
                aSuppr->suiv->prec = aSuppr->prec;
                aSuppr->prec->suiv = aSuppr->suiv;
            ...
            {
                decroitValcase(aSuppr->Qdata->pos[j]);
            }
...
bool checkVictory(listequintuplet l)
{
    ...
    while (parcours != NULL)
    {
        if(parcours->Qdata->valquintuplet >= 500) //valeur à modifier precisement, valeur maximale possible, cas d'un quintuplet à 5 symboles
            return true;
