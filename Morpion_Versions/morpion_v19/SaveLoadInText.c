...
{
    fprintf(fd, "Pos %d %d %d %d %d %d %d\n", pos->X, pos->Y, pos->valeur, pos->valX, pos->valO, pos->Sy, pos->partOfWinQuint);
}
...
{
    fprintf(fd, "Player %d %d %d\n", player.typejoueur, player.typeSymbole, player.winQuints);
...
{
    int X, Y, valeur, valX, valO, Sy, partOfWinQuint;
    fscanf(fd, "Pos %d %d %d %d %d %d %d\n", &X, &Y, &valeur, &valX, &valO, &Sy, &partOfWinQuint);
    struct t_position pos = {X, Y, valeur, valX, valO, Sy, partOfWinQuint};
    return pos;
}
...
{
    int typejoueur, typeSymbole, winQuints;
    fscanf(fd, "Player %d %d %d\n", &typejoueur, &typeSymbole, &winQuints);
    QList listeGagnant = readLiteListQuintuplets(fd, allQuints);
    QList listePerdante = readLiteListQuintuplets(fd, allQuints);
    TPlayer p = {typejoueur, typeSymbole, listeGagnant, listePerdante, winQuints};
