...
void printderniercoup(TPlayer p , int X , int Y)
{
    const char* InitialeTypeSymbole[3]={"VIDE", "X" , "O"};
    printf("\n\n");
    printf("\t   \t\t\t     <> Le dernier coup a ete jouer par le joueur '%s' :\n",InitialeTypeSymbole[p.typeSymbole]);
    printf("\t  \t\t\t\t\t\t --> En X = %d\n",X);
    printf("\t  \t\t\t\t\t\t --> En Y = %d\n",Y);
}

void printgagnant(TPlayer p)
{
    int b = 219;
    const char* InitialeTypeSymbole[3]={"VIDE", "X" , "O"};
    printf("\n\n\n\n\n\n");
    ...
    printf(" \t  \t\t\t\t  %c%c                                   %c%c\n",b,b,b,b);
    printf(" \t  \t\t\t\t  %c%c Le Joueur '%s' a gagne bravo a lui %c%c\n",b,b,InitialeTypeSymbole[p.typeSymbole],b,b);
    printf(" \t  \t\t\t\t  %c%c                                   %c%c\n",b,b,b,b);
    printf(" \t  \t\t\t\t  %c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c\n",b,b,b,b,b,b,b,b,b,b,b,b,b,b,b,b,b,b,b,b,b,b,b,b,b,b,b,b,b,b,b,b,b,b,b,b,b,b,b);
    printf("\n\n\n\n\n\n");
}
...
void ajouteSymboleBOT(TabMorpion jeu, T_Postion *ChoosenPos, TPlayer p)
{
    ...
    int X = getXP(ChoosenPos);
    int Y = getYP(ChoosenPos);
    jeu[X][Y] = p.typeSymbole;
...
listequintuplet makeMainList() //Appel� qu'une fois en d�but de jeu pour faire la liste principale de quintuplets vides
{
                ...
                for (int y_direction = -1 ; y_direction <= 1 ; y_direction++)                                               //Regardera dans toutes les directions possibles
                {
                    if(existeQuintu(x_of_origin, y_of_origin, x_direction, y_direction))                                    //on omet les doublons pour l'instant
                    {
                        ...
                        *q = createQuintu(x_of_origin, y_of_origin, x_direction, y_direction);
                        mainList = ajoutEnFinLQuint(mainList, q); //pas r�ellement d'int�r�t, par rapport � T�te ou Fin vu que les premiers quintuplets seront generalement pris au milieu,
                    }                                             //� part pour la lisibilit� : on aura les coordonn�es (0,0) en premier
                }
            }
        }
    }
...
listequintuplet suppDoublons(listequintuplet mainList) //Idem, appel� qu'une fois, juste apr�s makeMainList pour supprimer les doublons cr�es
{                                                      //Tri complexe et peu opti, je songeais au tri par hachage mais dur de l'adapter sur des structures
    listequintuplet courant, parcours;                 //Corrig� par Mr.MENEVEAUX Daniel
    bool found;
...
listequintuplet getAllQuints(T_Postion *pos, listequintuplet l)
{
    ...
    //On suppose l avec au moins un �l�ment (v�rifi� avant son appel)
    while (l != NULL) //on parcourt l'integralite de notre liste l
    {
        ...
        for (int i = 0 ; i < NTUPLET ; i++) //on compare chaque case du quintuplet avec notre case recherchee
            if (equalsP(pos, l->Qdata->pos[i]) && !found)
                found = true; //si on trouve une case identique, on arr�te le parcours du quintuplet, on passe � sa suppression sinon on passe au quintuplet suivant
        if (found == true){
...
void MiseAjourValQuint(T_Postion *pos, listequintuplet list){


    listequintuplet l=list;

    //On suppose l avec au moins un �l�ment (v�rifi� avant son appel)
    while (l != NULL) //on parcourt l'integralite de notre liste l
    {
        for (int i = 0 ; i < NTUPLET ; i++){ //on compare chaque case du quintuplet avec notre case recherchee
            if (equalsP(pos, l->Qdata->pos[i]))
		l->Qdata->valquintuplet = l->Qdata->valquintuplet + 100 ;
        }
	l = l->suiv; //et avancer le ptr courant
...
void affichevalquint(listequintuplet list){
listequintuplet l=list;

    //On suppose l avec au moins un �l�ment (v�rifi� avant son appel)
    while (l != NULL) //on parcourt l'integralite de notre liste l
    {
	printf("in the Game : %d\n",l->Qdata->valquintuplet);
	l = l->suiv; //et avancer le ptr courant
    ...
    while (courant != NULL)
    {
        ...
        while (parcoursL2 != NULL)
        {
            ...
            if (equals == true)
            {
                ...
                else //si c'est pas le cas, on fait la m�me chose diff�remment
                {
...
listequintuplet removeClosedQuints(listequintuplet l)
{

    listequintuplet courant;
    initListeQuint(&courant);
    ...
    while (l != NULL )
    {
        ...
        for (int i = 0 ; i < NTUPLET ; i++)
        {
            ...
            if (l->Qdata->pos[i]->Sy == rond){
                oExist = true;
            }

            if (l->Qdata->pos[i]->Sy == croix){
                xExist = true;
            }
        }
        if (oExist && xExist)
        {
            ...
            courant = ajoutEnTeteLQuint(courant,l->Qdata);
            l=l->suiv;
        }
...
void afficheQuintus(listequintuplet l) //Fonction d'affichage, uniquement utilis� pour le d�bogage manuel
{
    int i = 1;
...
bool checkVictory(listequintuplet l)
{
    ...
    while (parcours != NULL)
    {
        if(parcours->Qdata->valquintuplet >= 500) //valeur � modifier precisement, valeur maximale possible, cas d'un quintuplet � 5 symboles
            return true;
        else parcours = getptrNextCellLQuint(parcours);
    }

    return false; //on a pas trouv� de quintuplet � 5 symboles
}
...
int gameState(listequintuplet mainList, TPlayer p)
{
    if (listeVideLQuint(mainList) && listeVideLQuint(p.listeGagnant) && listeVideLQuint(p.listePerdante))
        return 2; //Partie avec plus de possibilit� de victoire des deux joueurs, cas le - probable, � mettre en dernier au switch
    else if(checkVictory(p.listeGagnant))
        return 1; //Partie avec la victoire du joueur, quintuplet � 5 symboles de trouv�, second cas le + probable
    else return 0; //Partie avec toujours une possibilit� de victoire, cas le + probable d'arriver donc � prioriser au switch apr�s
}
...
T_Postion *bestPos(listequintuplet list, Symboles symbole)
{
    Symboles sym = symbole;
    //printf("symbole de la liste :%d\n",sym);
    listequintuplet l = list;
    T_Postion *max_Pos = NULL;
    int valmaxPos = 0 ;
    ...
    while(!listeVideLQuint(l))
    {
        for (int i = 0 ; i < NTUPLET ; i++)
        {
            if ((getValP(l->Qdata->pos[i] , sym) + l->Qdata->valquintuplet > valmaxPos) && (l->Qdata->pos[i]->Sy == vide)){ // best pos sans symbole et en fonction de qui fait lappel ajout de getSymbole
                max_Pos = l->Qdata->pos[i];
                valmaxPos = getValP(l->Qdata->pos[i] , sym) + l->Qdata->valquintuplet;
                //printf("la val maxpos est : %d , (x= %d, y= %d) et le symbole est : %d\n", valmaxPos , getXP(l->Qdata->pos[i]),getYP(l->Qdata->pos[i]),sym);
            }
        }
...
int bestPosint(listequintuplet list , Symboles symbole)
{
   Symboles sym = symbole;
    listequintuplet l = list;
    int valmaxPos = 0 ;
...
void majValPosQuintuplet(listequintuplet list , Symboles symbole){
    ...
    while(listTest2 != NULL){
        for(int i = 0 ; i<NTUPLET ; i++){
            majValPosition(listTest2->Qdata->pos[i],listP,sym);
            }

        listTest2 = listTest2->suiv ;
    }

    liberationListPos(listP);
}
