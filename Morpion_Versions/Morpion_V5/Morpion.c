...
void afficheQuintus(listequintuplet l) //Fonction d'affichage, uniquement utilisé pour le débogage manuel
{
    ...
    else
    {
        while (!listeVideLQuint(courant))
        {
            ...
            {
                printf(" { %2d ; %2d ; %2d ; %2d } ", getXP(courant->Qdata->pos[j]), getYP(courant->Qdata->pos[j]), getValP(courant->Qdata->pos[j]), getSymboleP(courant->Qdata->pos[j]));
            }
            printf("]\n");
            i++;
            courant = getptrNextCellLQuint(courant);
...
listequintuplet getAllQuints(T_Postion *pos, listequintuplet* l)
{
    ...
    courant = *l;
    while (courant != NULL) //on parcourt l'integralite de notre liste l
    {
        found = false;
        for (int i = 0 ; i < NTUPLET ; i++) //on compare chaque case du quintuplet avec notre case recherchee
            if (equalsP(pos, courant->Qdata->pos[i]) && found == false)
        ...
        if (found == true)
        {
            result_list = ajoutEnTeteLQuint(result_list, courant->Qdata); //on transfere deja le quintuplet

            listequintuplet aSuppr = courant; //on declare et definit une variable locale qui va sauvegarder notre quintuplet a supprimer
            ...
            else if(aSuppr->prec == NULL) //si c'est le premier quintuplet de la liste, on pense a faire les manips necessaires
            {
