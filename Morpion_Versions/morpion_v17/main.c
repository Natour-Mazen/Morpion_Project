...
int main(int argc, char* argv[])
{
    ...
    if(continuJeu){
              ...
              !stopgame)
        {
            ...
            {
                txt_saveGame(playerX, playerO, mainList, allQuints, jeu, tabPosition);
                //b_saveGame(playerX, playerO, mainList, allQuints, jeu, tabPosition);
                messageSDL("SAUVEGARDE",">> La Sauvegarde de votre partie a ete effectue !");
            }
            // On charge une partie.
            if( pKeyStates[SDL_SCANCODE_L])
            {
                txt_loadGame(&playerX, &playerO, &mainList, &allQuints, jeu, &tabPosition);
                //b_loadGame(&playerX, &playerO, &mainList, &allQuints, jeu, &tabPosition);
